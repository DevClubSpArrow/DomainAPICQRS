...
namespace HindalcoBackend.Application.Models
{
    public class TokenGenerator
    {
        ...
        public class QueryHandler : IRequestHandler<Query, HindalcoBackend.Domain.DomainModels.AuthModels.AuthRefreshToken>
        {
            ...
            private HindalcoBackend.Domain.DomainModels.DataModels.UserModel umodel { get; }
            ...
            public async Task<HindalcoBackend.Domain.DomainModels.AuthModels.AuthRefreshToken> Handle(Query request,CancellationToken cancellationToken)
            {
                return umodel == null
                ? throw new InvalidOperationException("User model is null, cannot generate token.")
                : await _tokenService.Generatetoken(umodel);
            }
            public async Task<HindalcoBackend.Domain.DomainModels.AuthModels.AuthRefreshToken> Handle(AuthQuery request, CancellationToken cancellationToken)
            {
            ...
            public async Task<HttpResponseMessage> Handle(ValidateJwtQuery request, CancellationToken cancellationToken)
            {
            ...
            public async Task<HindalcoBackend.Domain.DomainModels.DataModels.AuditCalendar> Handle(GetAuditCalendar request, CancellationToken cancellationToken)
            {
                if (request.Id == null)
                {
